...
EOF
  ...
  provisioner "remote-exec" {
    inline = [
      "chmod +x $HOME/installation.sh; bash $HOME/installation.sh \"${var.mariadb_user}\" \"${var.mariadb_pwd}\" \"${vsphere_virtual_machine.php_vm.clone[0].customize[0].network_interface[0].ipv4_address}\"",
    ]
  }
}
